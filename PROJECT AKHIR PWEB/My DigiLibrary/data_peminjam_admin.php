...
$query = "SELECT * FROM detail_peminjaman JOIN buku USING (buku_id) JOIN peminjaman USING (peminjaman_id) JOIN staff USING (staff_id) JOIN member USING (member_id) ORDER BY detail_peminjaman_id ASC";
                                    ...
                                    <?php while ($row = $res1->fetch_assoc()) { ?>
                                        <tr>
                                            ...
                                            <td><?php if($row['status'] == 0){?>
                                                ...
                                                <a href="konfirmasi.php?id=<?= $row['detail_peminjaman_id'] ?>" onclick="return confirm('Yakin ingin mengkonfirmasi data ini?')" class="btn btn-primary">Konfirmasi</a>
                                                <?php }else{?> 
                                            ...
                                            <td>
                                                <span class="badge bg-warning"><a href="edit_peminjaman.php?id=<?= $row['detail_peminjaman_id'] ?>" class="text-light"><i class="fas fa-pen"></i> Edit</a></span><span class="badge bg-danger"><a href="hapus.php?id=<?= $row['detail_peminjaman_id'] ?>&amp;peminjaman_id=<?= $row['peminjaman_id'] ?>" class="text-light" onclick="return confirm('Yakin ingin menghapus data ini?')"><i class="fas fa-trash"></i> Hapus</a></span>
                                            </td>
                                        </tr>
                                    <?php } ?>
